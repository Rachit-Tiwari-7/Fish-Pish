export function calculateScore(signals: any, llmResult?: any) {
  let score = 0;
  const { safeBrowsing, ssl } = signals;

  // 1. Google Safe Browsing (+80)
  if (safeBrowsing.matches) score += 80;

  // 2. SSL
  if (!ssl.secure) score += 20;
  if (ssl.isSelfSigned) score += 15;
  if (ssl.ageDays < 7) score += 10;
  ...
  if (probability > 75) {
    color = "red";
    if (!llmResult?.reasoning) summary = "High risk detected! Avoid this link.";
  } else if (probability > 40) {
    color = "yellow";
    if (!llmResult?.reasoning) summary = "Suspicious indicators found. Proceed with caution.";
  }
